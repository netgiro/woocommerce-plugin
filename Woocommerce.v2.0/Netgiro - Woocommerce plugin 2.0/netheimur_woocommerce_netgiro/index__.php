...
function woocommerce_netheimur_netgiro_init(){
  ...
  class WC_netheimur_netgiro extends WC_Payment_Gateway{
    ...
    function init_form_fields(){
       ...
       $this -> form_fields = array(
                ...
                'description' => array(
                    ...
                    'default' => __('Reikningur sendur í netbanka, greiða þarf innan 18 daga eða Netgíró raðgreiðslur.', 'netheimur_netgiro')),
                'gateway_url' => array(
    ...
    function init_process_payment() {
      ...
      if($_GET['signature'] && $_GET['orderid'] && $_GET['confirmationCode'] ) {
          // Get order
          $order = new WC_Order( $_GET['orderid'] );

          // Create hash
          $str = $this->secretkey . $_GET['orderid'];
          $hash = hash('sha256', $str);

          // Check if signature is OK
          if($hash==$_GET['signature']) {

            // Complete order
            $order -> payment_complete();
            $order -> add_order_note('Netgíró payment successful<br/>InvoiceNumber from Netgíró: '.$_REQUEST['invoiceNumber']);
