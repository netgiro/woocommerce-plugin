  ...
  class WC_netheimur_netgiro extends WC_Payment_Gateway{
    ...
    function init_form_fields(){
       ...
       $this -> form_fields = array(
                ...
                'test' => array(
                    'title' => __( 'Prófunarumhverfi', 'netheimur_valitor' ),
                    'type' => 'checkbox',
                    'label' => __( 'Senda á prófunarumhverfi Netgíró', 'netheimur_netgiro' ),
                    'default' => 'option_is_enabled'),
                'rounding' => array(
                	'title' => __( 'Kommutölur', 'netheimur_valitor'),
                	'type' => 'checkbox',
                	'label' => __( 'Nota námundun til að fjarlægja kommutölur', 'netheimur_netgiro'),
                	'default' => 'no'
                	),
                /*'gateway_url' => array(
    ...
    public function generate_netgiro_form($order_id){
      ...
      $payment_Confirmed_url = add_query_arg( 'wc-api', 'WC_netheimur_netgiro', home_url( '/' ) );//$redirect_url;callback_function
      ...
      <form action="'.$this -> gateway_url.'" method="post" id="netheimur_payment_form">
          ...
          <a class="button cancel" href="'.$order->get_cancel_order_url().'">'.__('Hætta við greiðslu', 'netheimur_netgiro').'</a>
          </p>
    ...
    function init_process_payment() {
      ...
      if($_GET['ng_netgiroSignature'] && $_GET['ng_orderid'] && $_GET['ng_transactionid'] ) {
