  ...
  class WC_netheimur_netgiro extends WC_Payment_Gateway{
    ...
    public function generate_netgiro_form($order_id){
      ...
      foreach ($order->get_items() as $item) {
        ...
        'Name'=> $item['name'],
        'UnitPrice'=>($item['line_total']+$item['line_tax']) / $item['qty'],
    ...
    function init_process_payment() {
      ...
      //if($_GET['ng_netgiroSignature'] && $_GET['ng_referenceNumber'] && $_GET['ng_transactionId'] ) {
      if($_GET['ng_signature'] && $_GET['ng_orderid'] && $_GET['ng_confirmationCode'] ) {
          ...
          // Create hash
          $total = number_format($order->order_total, 0, '', '');
          //$str = $this->secretkey.$_GET['ng_referenceNumber'].$_GET['ng_transactionid'].$_GET['ng_invoiceNumber'].$_GET['ng_totalAmount'].$_GET['ng_status'];
          $str = $this->secretkey . $_GET['ng_orderid'];
          $hash = hash('sha256', $str);

          // Check if signature is OK
          //if($hash==$_GET['ng_netgiroSignature']) {
